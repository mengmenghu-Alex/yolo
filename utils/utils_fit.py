...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, optimizer, epoch, epoch_step, \
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            loss_value_all = 0
            for l in range(len(outputs)):
                loss_item = yolo_loss(l,outputs[1],targets)
                loss_value_all += loss_item
            loss_value = loss_value_all
    ...
    if local_rank == 0:
        ...
        if len(loss_history.val_loss)<=1 or(val_loss/epoch_step_val)<=min(loss_history.val_loss):
            print('Save best model to best_epoch_weights.path')
